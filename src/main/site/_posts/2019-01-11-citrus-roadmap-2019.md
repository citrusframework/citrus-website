...
We'll also create epics on GitHub for every major topic to make the progress transparent to the community. Please bear in mind that the dates
...
Over time the number of samples has grown so large that it's now overwhelming new uses.
That's why we want to review the Citrus samples completely and create a set that reflects the functionality of the framework
while making them easy to understand and a good point to start from. To achieve this, we'll focus on samples for the
...
We'll announce the final removal with at least one year of lead time for migration. Until then, the XML-DSL
...
In the long term we want to ship often and in small increments. In the future you won't have to wait as long for your 
...
we're going to deprecate the TestDesigner. We have been recommending the TestRunner for some time now and think, that the 
...
to ensure it's as easy as possible for you to create the infrastructure required by the framework.
...
integrated communication technology. We'll review the documentation's structure and content to make it easier to navigate
...
by reviewing the way we've setup our dependencies. The goal is to reduce the amount of required dependencies making the
...
As we're going to review and clean up our API in 2019, we'll also review the way test classes are created. Again the goal  
